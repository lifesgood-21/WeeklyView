...
class WeeklyView : FrameLayout {
    ...
    private fun fetchAttrs(typedArray: TypedArray) {
        defaultTextColor =
            typedArray.getColor(R.styleable.WeeklyView_wv_digit_color, getColor(R.color.vw_black))
        selectedDayTextColor = typedArray.getColor(
            R.styleable.WeeklyView_wv_selected_day_text_color,
            getColor(R.color.vw_white)
        )
        holidayTextColor =
            typedArray.getColor(R.styleable.WeeklyView_wv_holiday_color, getColor(R.color.vw_gray))
        todayTextColor = typedArray.getColor(
            R.styleable.WeeklyView_wv_today_color,
            getColor(R.color.vw_tangerine_two)
        )
        todayBackgroundColor = typedArray.getColor(
            R.styleable.WeeklyView_wv_today_background_color,
            getColor(R.color.vw_tangerine_two)
        )
        defaultBackgroundColor = typedArray.getColor(
            R.styleable.WeeklyView_wv_default_background_color,
            getColor(R.color.vw_black)
        )
        typedArray.getDrawable(R.styleable.WeeklyView_wv_next_week_res)
            ?.let { nextWeekDrawableRes = it }
        typedArray.getDrawable(R.styleable.WeeklyView_wv_prev_week_res)
            ?.let { prevWeekDrawableRes = it }
    }
    ...
    override fun onFinishInflate() {
        ...
        root = LayoutInflater.from(context)
            .inflate(R.layout.weekly_view, this, false) as RelativeLayout
        resetCalendar()
    ...
    private fun tuneDayHolder(index: Int) {
        ...
        with(dayHolder!!) {
            ...
            when (isToday) {
                true -> itemView.background?.setColorFilter(
                    todayBackgroundColor,
                    PorterDuff.Mode.SRC_ATOP
                )
                else -> itemView.background?.setColorFilter(
    ...
    private inner class WeeklyViewReadyToUpdateListener(private val fromX: Int) :
